<div class="page-header page-header-light">
	...
	<div class="ml-3 mr-3 mt-3 mb-3">
		<form action="{site_url}noakun/detail/{noakun}" id="form1" method="get">
			<div class="row">
				<div class="col-md-2">
				...
				<tbody>
					<?php if ($get_noakun): ?>
						...
						<?php foreach ($get_noakun as $row): ?>
							<?php $totaldebet = $totaldebet + $row['saldo'] ?>
							<?php $totalkredit = $totalkredit + $row['saldo'] ?>

							<tr>
								<td><?php echo $row['tanggal'] ?></td>
								<td><?php echo $row['keterangan'] ?></td>
								<td class="text-right"><?php echo formatnumberakunting($row['debet']) ?></td>
								<td class="text-right"><?php echo formatnumberakunting($row['kredit']) ?></td>
							</tr>
						<?php endforeach ?>
					<?php else: ?>
						<tr>
