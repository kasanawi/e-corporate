                        ...
                        <tbody>
                            <tr>
                                <td class="text-right" width="12%">
                                    <select name="itemid[]" class="form-control itemid" required style="width:100%"></select>
                                </td>
                                <td class="text-right" width="12%">
                                    <input type="text" name="harga[]" maxlength="9" class="form-control harga text-right decimalnumber" required>
                                </td>
                                <td class="text-right" width="12%">
                                    <input type="number" name="jumlah[]" maxlength="9" class="form-control jumlah text-right decimalnumber" required>
                                </td>
                                <td class="text-right" width="12%">
                                    <input type="text" name="subtotal[]" maxlength="9" class="form-control subtotal text-right decimalnumber" required>
                                </td>
                                <td class="text-right" width="12%">
                                    <input type="text" name="diskon[]" maxlength="9" class="form-control diskon text-right decimalnumber" required>
                                </td>
                                <td class="text-right" width="12%">
                                    <input type="text" name="ppn[]" maxlength="9" class="form-control ppn text-right decimalnumber" required>
                                </td>
                                <td class="text-right" width="12%">
                                    <input type="text" name="total[]" maxlength="9" class="form-control total text-right decimalnumber" required>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot class="bg-light">
                            <tr>
                                <th class="text-center">
                                    <button type="button" class="btn btn-sm add_row {bg_header}"><?php echo lang('add_row') ?></button>
                                </th>
                                <th colspan="5" class="text-right"><?php echo lang('grand_total') ?></th>
...
<script type="text/javascript">
    ...
    $('#table_detail tbody').on('change','.itemid',function(){
        var itemid = $(this).val();
        var data;
        $.ajax({
            url: base_url + 'detailitem',
            dataType: 'json',
            method: 'post',
            data: { itemid: itemid },
            async: false,
        ...
        var itemid, harga, jumlah, subtotal, diskon, ppn, total, nominaldiskon, nominalppn, jumlahsisa = null;
        var row = $(this).closest('tr');
        row.find('input.harga').val( numeral(data.hargabeliterakhir).format() );
        row.find('input.jumlah').val( numeral(1).format() );
        row.find('input.subtotal').val( numeral(data.hargabeliterakhir).format() );
        row.find('input.diskon').val( numeral(0).format() );
        row.find('input.ppn').val( numeral(0).format() );
        row.find('input.total').val( numeral(data.hargabeliterakhir).format() );
    ...
    $('#table_detail').on('click','.add_row',function(){
        var clonerow = $('tbody tr:last').clone();
        $('#table_detail tbody').append(clonerow);
