...
<script type="text/javascript">
        ...
        if ('<?= $this->session->userid; ?>' !== '1') {
            ...
            $.ajax({
                ...
                success : function (response) {
                    ...
                    // $("[id^=checkbox_PKK]").attr('setupJurnal', response.kodeJurnal);
                    // $("[id^=checkbox_PKK]").attr('idSetupJurnal', response.idSetupJurnal);
                }
            })
        } else {
          ajax_select({
    ...
    var table_detail = $('#table_detail_rincian_buku_kas_umum').DataTable({
        ...
        drawCallback: function(dt) {
          ...
          $('.pilihRekening').select2({tags: true});
        }
    })
    ...
    function save_detail(elem, edit) {
      ...
      if (edit) { 
        ...
        // let idSetupjurnal;
        // let setupJurnal;
        var idSetupJurnal     = $(elem).attr('idSetupJurnal');
        var setupJurnal       = $(elem).attr('setupjurnal');
      ...
      if (tipe == 'Penjualan'){  
        ...
        if (stat) {
          table_detail.row.add([
            `${id}`,
            `<button type="button" class="btn btn-danger delete_detail" id="buttonPiutang${id}" data-id="${id}" data-idkas="${id}" data-tipe="${tipe}" onclick="hapus_data(this);">-</button>`,
            `${tipe}`,
            `${tgl}`,
            `${nokwitansi}`,
            formatRupiah(String(nominal))  + ',00',
        ...
        var rekening_bank = '';
        var pilihRekening_html = '';
        var html = '';
        for (i = 0; i < listRekening.length; ++i) {
          html += '<option value="' + listRekening[i].id + '"';
          if(listRekening[i].id==idRekening) rekening_bank = listRekening[i].nama; html += ' selected';
          html += '>' + listRekening[i].nama + '</option>';
        }

        var pilihRekening_html = '';
        // console.log(idRekening, 'idRekening')
        if( typeof idRekening !== 'undefined' ) {
          pilihRekening_html = `<input type="hidden" name="xidRekening[]" value="${idRekening}" id="idRekening${id}">${rekening_bank}`;
        } else {
          pilihRekening_html = `<input type="hidden" name="xidRekening[]" value="${idRekening}" id="idRekening${id}"><select onchange="pilihRekening(this, 'idRekening${id}')" name="idRekening[]" class="form-control xpilihRekening" required>${html}</select>`;
        }
