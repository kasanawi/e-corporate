...
class Penerimaan_pembelian extends User_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Penerimaan_pembelian_model','model');
	}

	public function index() {
		$data['title'] = lang('goods_receipt');
		$data['subtitle'] = lang('list');
		$data['content'] = 'Penerimaan_pembelian/index';
	...
	public function create($id) {
		...
		$data['content']				= 'Penerimaan_pembelian/create';
		$data['Penerimaan']				= $this->model->get($id);
		...
		$data['Penerimaan']['detail']	= $this->db->get_where('tpemesanandetail', [
			'idpemesanan'	=> $data['Penerimaan']['idpemesanan']
		])->result_array();
		$data['Penerimaan']['log'] = $this->db->last_query();
		$data = array_merge($data,path_info());
		$this->parser->parse('template',$data);
	}
	...
	public function detail($id = null) {
		if($id) {
			$data = $this->model->getPenerimaan($id);
			if($data) {
				$data['kontak'] = get_by_id('id',$data['kontakid'],'mkontak');
				$data['gudang'] = get_by_id('id',$data['gudangid'],'mgudang');
				$data['Penerimaandetail'] = $this->model->Penerimaandetail($data['id']);
				$data['title'] = lang('goods_receipt');
				$data['subtitle'] = lang('detail');
				$data['content'] = 'Penerimaan_pembelian/detail';
				$data = array_merge($data,path_info());
	...
	public function printpdf($id = null) {
		...
		$data = array_merge($data,path_info());
		$html = $this->load->view('Penerimaan_pembelian/printpdf', $data, TRUE);
		$pdf->loadHtml($html);
		$pdf->setPaper('A4', 'portrait');
		$pdf->render();
		$time = time();
		$pdf->stream("Penerimaan-pembelian-". $time, array("Attachment" => false));
	}
	...
	public function edit($id = null) {
		if($id) {
			...
			if($data) {
				...
				$data['subtitle'] 				= lang('detail');
				$data['tanggal'] 				= date('Y-m-d');
				$data['content']				= 'Penerimaan_pembelian/edit';
				$data['Penerimaan']				= $this->model->get($id);
				$this->db->select('tpemesanandetail.*, mitem.nama as nama_barang, mitem.kode');
				$this->db->join('tanggaranbelanjadetail', 'tpemesanandetail.itemid = tanggaranbelanjadetail.id');
				$this->db->join('mitem', 'tanggaranbelanjadetail.uraian = mitem.id');
	...
	public function validasi($status= null, $id = null)
	{
		...
		redirect(base_url('Penerimaan_pembelian'));	
	}
	...
	{
		$data	= $this->model->get_penerimaan_item(null, $kontak);
		$no		= 0;
		foreach ($data as $key) {
			$data0[$no]['id']	= $key['id'];
			$data0[$no]['text']	= $key['notrans'] . ' - ' . $key['tanggal_Penerimaan'] . ' -A ' . $key['supplier'] . ' - ' . number_format($key['nominal_penerimaan'],2,',','.');
			$no++;
		}	
		$this->output->set_content_type('application/json')->set_output(json_encode($data0));
	}
	
	public function select4($kontak = null) // untuk uji fungsi select2
	{
		$data	= $this->model->get_penerimaan_item(null, $kontak);
		$no		= 0;
		foreach ($data as $key) {
			$data0[$no]['id']	= $key['id'];
			$data0[$no]['text']	= $key['notrans'] . ' - ' . $key['tanggal_Penerimaan'] . ' -N- ' . $key['supplier'] . ' - ' . number_format($key['nominal_penerimaan'],2,',','.');
