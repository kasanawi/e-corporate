...
class Pembayaran_penjualan extends User_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Pembayaran_penjualan_model','model');
	}

	public function index() {
		$data['title'] = lang('pembayaran_penjualan');
		$data['subtitle'] = lang('list');
		$data['content'] = 'Pembayaran_penjualan/index';
	...
	public function index_datatable() {
		...
		$this->datatables->where('tpembayaran.tipe','2');
	...
	public function create() {
		...
		$data['totalkreditmemo'] = $this->model->gettotalkreditmemo($data['kontakid']);
		$data['title'] = lang('payment');
		$data['subtitle'] = lang('add_new');
		$data['content'] = 'Pembayaran_penjualan/create';
		$data = array_merge($data,path_info());
		$this->parser->parse('default',$data);
	}
	...
	public function detail($id = null) {
		if($id) {
			...
			$data['content'] = 'Pembayaran_penjualan/detail';
			$data = array_merge($data,path_info());
			$this->parser->parse('default',$data);
		}
	}
	...
	public function printpdf($id = null) {
		...
		$data['fakturdetail'] = $this->model->getfakturdetail($data['id']);
	    $data['title'] = 'Pembayaran Penjualan';
	    $data['css'] = file_get_contents(FCPATH.'assets/css/print.min.css');
	    $data = array_merge($data,path_info());
	    $html = $this->load->view('Pembayaran_penjualan/printpdf', $data, TRUE);
	    $pdf->loadHtml($html);
	    $pdf->setPaper('A4', 'landscape');
	    $pdf->render();
	    $time = time();
	    $pdf->stream("pembayaran-penjualan-". $time, array("Attachment" => false));
	}
	...
	public function create_memo() {
		...
		$data['title'] = lang('payment');
		$data['subtitle'] = lang('add_new');
		$data['content'] = 'Pembayaran_penjualan/create_memo';
		$data = array_merge($data,path_info());
		$this->parser->parse('default',$data);
	}
