...
class Retur_penjualan extends User_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model('Retur_penjualan_model','model');
	}
	...
	public function index_datatable() {
		...
		$this->datatables->where('tretur.tipe','2');
	...
	public function create() {
		...
		if($idfaktur) {
			...
			$data['content'] = 'Retur_penjualan/create';
			$data['title'] = lang('return');
	...
	public function printpdf($id = null) {
	    ...
	    $data = array_merge($data,path_info());
	    $html = $this->load->view('Retur_penjualan/printpdf', $data, TRUE);
	    $pdf->loadHtml($html);
	    $pdf->setPaper('A4', 'portrait');
	    $pdf->render();
	    $time = time();
	    $pdf->stream("retur-penjualan-". $time, array("Attachment" => false));
	}
	...
	public function select2_item($id = null) {
		...
		if($id) {
			...
			$this->db->join('mitem', 'mitem.id = tstokkeluar.itemid', 'left');
			$this->db->where('tstokkeluar.refid', $faktur['pengirimanid']);
			$data = $this->db->where('mitem.id', $id)->get('tstokkeluar')->row_array();
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
		} else {
			$this->db->select('mitem.id, mitem.nama as text');
			...
			$data = $this->db->get('tstokkeluar')->result_array();
			$this->output->set_content_type('application/json')->set_output(json_encode($data));
		}
	}
